            ...
            <?php foreach($data['requests'] as $row) :?>
                <div class="form">
                    <div class="container request-container">
                        <p>Game ID: <?=$row->game_name?> |</p>
                        <p>Critic Review Request</p>
                        <div class="accept">
                            <h4>Organization:</h4>
                            <input type="text" class="input" value="<?=$row->critic?>">
                            <h4>Email:</h4>
                            <input type="email" class="input" value="<?=$row->email?>">
                            <h4>Game Score:</h4>
                            <input type="number" class="input" min="0" max="50" value="<?=$row->score?>">
                            <h4>Article Link:</h4>
                            <input type="url" class="input" value="<?=$row->link?>">
                            <h4>Comments on the Game:</h4>
                            <textarea id="ta"><?=$row->comment?></textarea>
                            <form class="submit-req" method="POST">
                                <input type="hidden" name="comment_id" value="<?=$row->comment_id?>">
                                <input type="submit" name="decision" class="form-btn" value="reject">
                                <input type="submit" name="decision" class="form-btn" value="accept">
                            </form>
                        </div>
                    </div>
                </div>
